...
def save_pickle(obj: Any, file_name: str) -> None:
...
def save_csv(data: pd.DataFrame, file_name: str, **kwargs: dict) -> None:
    '''Salvar um DataFrame em formato CSV.
    ...
    file_name: str
        Nome do arquivo a ser gerado.
    **kwargs: dict
        Argumentos do método 'to_csv'.
    '''
    if not os.path.isdir(os.path.dirname(file_name)):
        os.makedirs(os.path.dirname(file_name))

    data.to_csv(file_name, index=False, **kwargs)
...
def calculate_psi(
    ...
    Params
    ------
    ref_vals: pandas.Series
        Distribuição de referência.
    curr_vals: pandas.Series
        Distribuição atual.
    bins: int
    ...
    '''
    # Discretizar os valores numéricos com base na distribuição de referência
    ...
    # Contar os valores de uma variável categórica
    else:
        ref_counts = ref_vals.value_counts().sort_index()
        curr_counts = ref_vals.value_counts().sort_index()
